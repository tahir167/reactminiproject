...
const ClientFormforPay = () => {
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        ...
        if (currentBalance < totalOrderPrice) {
          enqueueSnackbar(`Balansda pul yoxdur! Balansınız: $${currentBalance.toFixed(2)}, Lazım olan: $${totalOrderPrice.toFixed(2)}`, {
            variant: 'error', autoHideDuration: 4000, anchorOrigin: { vertical: 'bottom', horizontal: 'right' }
          });
          return;
        }
        ...
        if (response.status === 201 || response.status === 200) {
          // Balance azaldılır
          const newBalance = (currentBalance - totalOrderPrice).toFixed(2);
          await controller.update(endpoints.users, user.id, { balance: parseFloat(newBalance) });
          dispatch(updateUser({ balance: parseFloat(newBalance) }));
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="w-[90%] lg:w-[80%] mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8"> 
        <div className="lg:col-span-2 bg-white p-4 sm:p-8 rounded-lg shadow-md">
          <h1 className="text-2xl sm:text-3xl font-bold text-gray-800 mb-6 text-center">
            Çatdırılma Məlumatları
          </h1>

          {/* Balance xəbərdarlığı */}
          <div className={`mb-4 p-3 rounded-md text-sm font-medium ${
            parseFloat(user.balance) >= totalOrderPrice
              ? 'bg-green-50 text-green-700 border border-green-200'
              : 'bg-red-50 text-red-700 border border-red-200'
          }`}>
            Balansınız: <strong>${parseFloat(user.balance || 0).toFixed(2)}</strong>
            {totalOrderPrice > 0 && parseFloat(user.balance) < totalOrderPrice && (
              <span className="ml-2">— Balansda pul yoxdur!</span>
            )}
          </div>

          <form onSubmit={formik.handleSubmit} className="space-y-6">
            <div>
              <label htmlFor="country" className="block text-sm font-medium text-gray-700 mb-1">Ölkə *</label>
              <input
                type="text" id="country" name="country"
                value={formik.values.country} onChange={formik.handleChange} onBlur={formik.handleBlur}
                className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${formik.touched.country && formik.errors.country ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
                placeholder="Məsələn: Azərbaycan"
              />
              {formik.touched.country && formik.errors.country && <p className="mt-1 text-sm text-red-600">{formik.errors.country}</p>}
            </div>

            <div>
              <label htmlFor="city" className="block text-sm font-medium text-gray-700 mb-1">Şəhər *</label>
              <input
                type="text" id="city" name="city"
                value={formik.values.city} onChange={formik.handleChange} onBlur={formik.handleBlur}
                className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${formik.touched.city && formik.errors.city ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
            ...
            <div>
              <label htmlFor="streetAddress" className="block text-sm font-medium text-gray-700 mb-1">Küçə ünvanı *</label>
              <input
                type="text" id="streetAddress" name="streetAddress"
                value={formik.values.streetAddress} onChange={formik.handleChange} onBlur={formik.handleBlur}
                className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${formik.touched.streetAddress && formik.errors.streetAddress ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
                placeholder="Məsələn: Nizami küçəsi 5"
              />
              {formik.touched.streetAddress && formik.errors.streetAddress && <p className="mt-1 text-sm text-red-600">{formik.errors.streetAddress}</p>}
            </div>

            <div>
              <label htmlFor="phoneNumber" className="block text-sm font-medium text-gray-700 mb-1">Telefon nömrəsi *</label>
              <input
                ...
                className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${formik.touched.phoneNumber && formik.errors.phoneNumber ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
                placeholder="Məsələn: 0501234567"
              />
              {formik.touched.phoneNumber && formik.errors.phoneNumber && <p className="mt-1 text-sm text-red-600">{formik.errors.phoneNumber}</p>}
            </div>

            <div>
              <label htmlFor="orderNote" className="block text-sm font-medium text-gray-700 mb-1">Sifariş qeydi *</label>
              <textarea
                id="orderNote" name="orderNote" rows={4}
                value={formik.values.orderNote} onChange={formik.handleChange} onBlur={formik.handleBlur}
                className={`w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 ${formik.touched.orderNote && formik.errors.orderNote ? 'border-red-500 focus:ring-red-500' : 'border-gray-300'}`}
                placeholder="Sifarişinizlə bağlı qeydlərinizi yazın..."
              />
              {formik.touched.orderNote && formik.errors.orderNote && <p className="mt-1 text-sm text-red-600">{formik.errors.orderNote}</p>}
            </div>

            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <button type="button" onClick={() => navigate('/basket')} className="flex-1 py-3 px-4 border border-gray-300 rounded-md text-gray-700 hover:bg-gray-50 transition-colors">
                Geri qayıt
              </button>
              <button
                type="submit"
                disabled={formik.isSubmitting || !formik.isValid || detailedBasketItems.length === 0 || parseFloat(user.balance) < totalOrderPrice}
                className={`flex-1 py-3 px-4 rounded-md text-white font-semibold transition-colors ${
                  formik.isSubmitting || !formik.isValid || detailedBasketItems.length === 0 || parseFloat(user.balance) < totalOrderPrice
        ...
        <div className="lg:col-span-1 bg-white p-4 sm:p-6 rounded-lg shadow-md h-fit">
          ...
          {detailedBasketItems.map((item) => (
            <div key={item.id} className="flex items-center justify-between border-b py-3 last:border-b-0">
              ...
              <p className="font-bold text-gray-800 text-sm sm:text-base">${(item.price * item.quantity).toFixed(2)}</p>
            </div>
          ))}
